...
describe('ZakoSecretCommand', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(randomUtils, 'getRandomString').mockReturnValue('Ab1Cd2Ef3G');
  });
  ...
  it('コマンドのプロパティが正しいこと', () => {
    ...
    expect(command.getExamples('zakosecret')).toHaveLength(2);
  });
  ...
  it('デフォルトで10文字の文字列を生成すること', async () => {
    ...
    expect(mockSay).toHaveBeenCalledWith({
      text: '🔑 生成されたシークレット文字列: `Ab1Cd2Ef3G`',
  ...
  it('指定された長さで文字列を生成すること', async () => {
    ...
    expect(randomUtils.getRandomString).toHaveBeenCalledWith(20);
  });
